...
> 사전 활성화: [Week1 Day1&2 - KServe RawDeployment 구성](<Week1-Day1&2-환경구성.md#kserve-rawdeployment-구성>)을 먼저 확인한다.
...
cd /tmp
rm -rf /tmp/wheelhouse-cp39-day5
mkdir -p /tmp/wheelhouse-cp39-day5
...
rm -rf /tmp/day05-pypi-upload-venv
/usr/bin/python3 -m venv /tmp/day05-pypi-upload-venv
source /tmp/day05-pypi-upload-venv/bin/activate
...
twine upload \
  ...
  /tmp/wheelhouse-cp39-day5/*

## 내부 nexus 이용해서 모델 생성
deactivate 2>/dev/null || true
cd /tmp/python3
rm -rf /tmp/python3/.venvs/day05
mkdir -p /tmp/python3/.venvs
/usr/bin/python3 -m venv /tmp/python3/.venvs/day05
source /tmp/python3/.venvs/day05/bin/activate
...
### 장애 시 blue로 롤백
green에서 오류가 발생했다고 가정하고 기존 blue로 모든 트래픽을 즉시 되돌린다. InferenceService를 먼저 삭제하지 않고 Route weight만 복구해야 endpoint를 유지할 수 있다.

```bash
oc patch route fraud-route -n jukebox --type=merge \
  -p '{"spec":{"port":{"targetPort":"http"},"to":{"kind":"Service","name":"fraud-blue-predictor","weight":100},"alternateBackends":[{"kind":"Service","name":"fraud-green-predictor","weight":0}]}}'

sleep 10

oc get route fraud-route -n jukebox -o wide
...
blue prediction만 반환되면 rollback이 완료된 것이다. 다음 실습을 위해 기존 90:10 상태를 유지하려면 같은 patch에서 weight를 `90`과 `10`으로 다시 적용한다.
