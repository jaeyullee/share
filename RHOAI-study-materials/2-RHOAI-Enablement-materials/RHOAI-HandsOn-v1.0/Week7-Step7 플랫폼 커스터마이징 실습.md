...
## week 7 - Step 7 플랫폼과 Dashboard 커스터마이징

> 사전 준비: [Week7 Step0](<Week7-Step0 사전점검 실습.md>)에서 `OdhDashboardConfig`와 `rhods-dashboard` Deployment를 백업한다. SSH를 다시 접속한 경우 아래 블록에서 정확한 백업 디렉터리를 다시 선택한다.
...
test -s "$WEEK7_BACKUP_DIR/odh-dashboard-config.json"
test -s "$WEEK7_BACKUP_DIR/rhods-dashboard.json"
