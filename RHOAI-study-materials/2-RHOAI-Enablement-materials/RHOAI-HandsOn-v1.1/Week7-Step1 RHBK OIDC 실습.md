...
```bash
test -f "$WEEK7_BACKUP_DIR/oauth.json"
...
5. 로그아웃한 뒤 `rhoai-week7-user`로 로그인해 일반 프로젝트 기능은 보이지만 관리자 Settings 변경 권한은 없는지 확인한다.
...
unset RHBK_HOST RHBK_ISSUER APPS_DOMAIN OAUTH_REDIRECT
```
