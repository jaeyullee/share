...
## 준비한 오퍼레이터 목록 (홈서버 기준 RHOAI 기능테스트에 필요할 것 같은 모든 오퍼레이터를 준비한거라 실제로는 필요하지 않거나, 누락된 게 있을 수 있음)
